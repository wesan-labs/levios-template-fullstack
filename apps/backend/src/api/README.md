...
An API Route is created in a TypeScript or JavaScript file under the `/src/api` directory of your Levios application. The file's name must be `route.ts` or `route.js`.

> Learn more about API Routes in [this documentation](https://docs.leviosjs.com/learn/fundamentals/api-routes)
...
export async function POST(req: LeviosRequest, res: LeviosResponse) {
  // Handle POST requests
}

export async function PUT(req: LeviosRequest, res: LeviosResponse) {
  // Handle PUT requests
}
```
...
import type {
  LeviosRequest,
  LeviosResponse,
} from "@wesan-labs/levios-framework/http"
...
The Levios container is available on `req.scope`. Use it to access modules' main services and other registered resources:
...
```ts
import { defineMiddlewares } from "@wesan-labs/levios-framework/http"
import type {
  LeviosRequest,
  LeviosResponse,
  LeviosNextFunction,
